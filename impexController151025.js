...
async function createVesselWithInspectionAndScores(newScoredData, user, data, reportDate, fileName, vesselsOperationData, vesselPortCountry, vesselsInspectingCompanyData) {
  ...
  for (const item of newScoredData) {
    ...
    const parseTmsaFromSoc = (soc) => {
      ...
      // Match patterns like "1A", "1A.1", "4A.2.1.3", "3.1.2", etc.
      const match = soc.match(/(\d+[A-Z]?(?:\.\d+){0,3})/i);

      return match ? match[1].toUpperCase() : null; // Return the first match, uppercase if letter present
    };
    ...
    const fetchScoreData = (negatives, category, isPositive) => {
      ...
      for (const neg of negatives) {
        ...
        const operatorComments = isOperatorComments ? neg?.operatorComments : filteredOperatorComments?.length > 0 ? filteredOperatorComments : null
...
//   const isHumanHdr = (line) => line.trim().toLowerCase().startsWith(HUMAN.toLowerCase());
//   const isOpHdr = (line) => /^operator\s+comments?:?/i.test(line.trim());

//   // compile NOC regexes once
//   const nocRegexes = humanNocs.map(noc =>
//     new RegExp(`^(\\d+\\.\\s*)?\\s*${escapeRegExp(noc)}\\b`, 'i')
//   );
//   const matchNoc = (line) => {
//     const t = line.trim();
//     for (const rx of nocRegexes) {
//       const m = t.match(rx);
//       if (m) return { matched: true, text: m[0].trim() };
//     }
//     return { matched: false, text: "" };
//   };

//   for (let i = 0; i < filterQuestion.length; i++) {
//     const header = (filterQuestion[i].data ?? "").trim();
//     if (!isHumanHdr(header)) continue;

//     // ---- parse SOC + negative from Human header ----
//     let soc = "";
//     let negative = "";
//     const content = header.replace(/^Human\s*/i, "").replace(/\(.*?\)/g, "").trim();

//     if (content.includes(":")) {
//       const [sPart, nPart] = content.split(":").map((x) => x.trim());
//       soc = sPart;
...
//     // ---- collect the full Human block (until next Human) ----
//     const block = [];
//     let j = i + 1;
//     for (; j < filterQuestion.length; j++) {
//       const line = (filterQuestion[j].data ?? "").trim();
//       if (isHumanHdr(line)) break;
//       block.push(line);
//     }

//     // ---- find the 'leading remark' (everything from start of block up to the FIRST NOC or Operator Comments) ----
//     const leadingRemarkLines = [];
//     for (let k = 0; k < block.length; k++) {
//       const line = block[k];
//       if (matchNoc(line).matched) break;   // stop at first NOC
//       if (isOpHdr(line)) break;            // or stop if Operator Comments appears before any NOC
//       if (line !== "") leadingRemarkLines.push(line);
//     }
//     const leadingRemark = leadingRemarkLines.join(" ").trim();

//     // ---- emit one entry per NOC IN THIS BLOCK, all using the SAME leading remark ----
//     for (let k = 0; k < block.length; k++) {
//       const m = matchNoc(block[k]);
//       if (m.matched) {
//         result.push({
//           soc,
//           negative,
//           remark: leadingRemark, // <-- same remark for all NOCs in this Human block
//           noc: m.text
//         });
//       }
//     }
...
function extractValidHumanNegatives(filterQuestion, nocGrouped, negativeObsertions) {
  ...
  const isHumanHdr = line => CLEAN(line).toLowerCase().startsWith(HUMAN.toLowerCase());
  const isOpHdr    = line => /^operator\s+comments?:?/i.test(CLEAN(line));
  const isSection  = line => /^(hardware|process|human|photograph)\b/i.test(CLEAN(line));

  // STRICT NOC detector: standalone header (optional number/bullet + exact NOC + optional punctuation)
  const nocRegexesStrict = humanNocs.map(noc =>
    new RegExp(`^\\s*(?:\\d+\\.?\\s*|[\\u2022*-]\\s*)?${escapeRegExp(noc)}\\s*[.;:!?']?\\s*$`,'i')
  );
  ...
  function parseOC(lines){
    ...
    const oc = { name:'', date:'', immediateCause:'', rootCause:'', rootCauseItems:[], correctiveAction:'', preventativeAction:'', notes:'' };
    let field = '', buf = '';
  ...
  for(let i=0;i<filterQuestion.length;i++){
    ...
    // Collect lines for this Human block
    const raw = [];
    let j=i+1;
    for(; j<filterQuestion.length; j++){
      const txt = CLEAN(filterQuestion[j].data);
      if(isHumanHdr(txt)) break;
      raw.push({ text: txt, idx: j });
    }

    // Stream with OC state
    ...
    // ONLY Root Cause header triggers suppression, and it protects the NEXT NON-EMPTY line
    const ROOT_HDR_RE = /^(?:\d+[.)-]|\u2022|\*|-)?\s*root\s*cause(?:s|\(s\))?(?:\s*analysis)?\s*[:\-]?\s*(?:not\s*applicable|\S.*)?$/i;
    let suppressNextNonEmptyNoc = false;
    ...
    for (let r = 0; r < raw.length; r++) {
      ...
      if (inOC) {
        // hard boundaries (no need for "operator attachments")
        if (isSection(t)) { endOC(r); r--; continue; }

        // flag suppression when we see a Root Cause header line
        if (ROOT_HDR_RE.test(t)) {
          suppressNextNonEmptyNoc = true;
          ocLines.push(t);
          block.push({ ...raw[r], type: 'op_content' });
          continue;
        }

        // if blank while waiting for the next non-empty → keep waiting (do NOT consume)
        if (suppressNextNonEmptyNoc && t === '') {
          ocLines.push(t);
          block.push({ ...raw[r], type: 'op_content' });
          continue;
        }

        const nocLike = matchNocStrict(t).matched;

        if (nocLike && suppressNextNonEmptyNoc) {
          // this is the first non-empty line after Root Cause → keep inside OC and consume protection
          suppressNextNonEmptyNoc = false;
          ocLines.push(t);
          block.push({ ...raw[r], type: 'op_content' });
          continue;
        ...
        if (nocLike) {
          // real boundary: end OC BEFORE this line so it becomes a true NOC
          endOC(r);
        ...
        ocLines.push(t);
        block.push({ ...raw[r], type: 'op_content' });
        // any non-empty line (that wasn't protected) cancels the suppression
        if (suppressNextNonEmptyNoc && t !== '') suppressNextNonEmptyNoc = false;
...
function extractTMSA(filterQuestion) {
  // CORRECTED REGEX: Captures the full TMSA reference in group 1
  // (\d+[A-Z]?(?:\.\d+){0,3}) matches 1A, 1A.1, 1A.1.1, and 1A.1.1.2
  const tmsaRegex = /(\d+[A-Z]?(?:\.\d+){0,3})/;
  ...
  for (let i = 0; i < filterQuestion.length; i++) {
    ...
    // 1. Check for the format 'TMSA - Description'
    // Uses the regex as a capturing group, followed by the separator
    const tmsaMatch = line.match(new RegExp(`^${tmsaRegex.source}\\s*-\\s*(.+)`));
    if (tmsaMatch) {
      // tmsaMatch[1] now contains the full reference, e.g., '1A.1.1.2'
      return tmsaMatch[1]; 
    }

    // 2. Check for the standalone TMSA number on its own line
    const standaloneTmsaMatch = line.match(new RegExp(`^${tmsaRegex.source}$`));
    if (standaloneTmsaMatch) {
      // standaloneTmsaMatch[1] now contains the full reference, e.g., '1A.1.1.2'
      return standaloneTmsaMatch[1]; 
    }
