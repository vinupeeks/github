...
const createTrial = async (req, res) => {
    try {
        ...
        if (fullVersion) {
            ...
            await createDemoNoRollback({ 
                userId: newUser.id,
                inspectionQuestions: inspectionQuestions, 
                inspectionScores: inspectionScores,
                graphNotesData: demoGraphNotes,
            })

            // Second Demo Inspection (New)
            await createDemoNoRollback({
                userId: newUser.id,
                vesselName: 'Demo Vessel 2', 
                reportDate: '2025-10-30', 
                inspectionQuestions: inspectionQuestions2,
                inspectionScores: inspectionScores2,
            })
        }
...
const sendMail = async ({ fullname, company_name, email, password, fullVersion }) => {
    try {
        ...
        const filePath = path.resolve(__dirname, '../../files/quick_guide/Quick-start.pdf');
        const stat = fs.statSync(filePath);
        if (!stat.isFile() || stat.size === 0) throw new Error('Local PDF missing or empty');
...
async function createDemoNoRollback({
    ...
    // 3) Graph Notes
     if (graphNotesData.length > 0) {
        const graphNotesPayload = graphNotesData.map(note => ({
            user_id: userId,
            ...note,
        }));
        await GraphNotes.bulkCreate(graphNotesPayload);
    }
    ...
    const scoresPayload = inspectionScores.map(s => {
        ...
        return {
            ...
            pif: s.pif || null,
            tmsa: s.tmsa || null,
            human_name: s.human_name || null,
            isWrong: s.isWrong || null,
            crew_id: s.crew_id
        }
    })
...
const demoGraphNotes = [
    {
        name: 'obs_per_insp_bar',
        note: '<p><span style="background-color: rgb(187, 187, 187);">2 Sire</span> inspections were performed from Jan the <span style="background-color: rgb(187, 187, 187);">1st to 30 June 2025 </span>resulting in <span style="background-color: rgb(187, 187, 187);">11</span> observations. During 2025, the observations per</p><ol><li data-list="bullet"><span class="ql-ui" contenteditable="false"></span>inspection under SIRE 2.0 was 4.55 obs/insp.</li><li data-list="bullet"><span class="ql-ui" contenteditable="false"></span>Average observations per inspection for this period was <span style="background-color: rgb(187, 187, 187);">90.91</span> obs/insp.</li></ol><p><br></p><p>No available official data for market’s average, just an indication from Intertanko giving an average of 2 obs/Insp</p><p>KPI set for 2025 was to perform better than industry’s average, which till now is achieved.</p>',
        status: 'active',
    },
    {
        name: 'ques_type_with_category',
        note: '<p>1 out of 11 observations were erroneously raised by the inspectors, the percentage is <span style="background-color: rgb(187, 187, 187);">9.01%</span>. The wrong observations in 2024 were <span style="background-color: rgb(187, 187, 187);">18%</span>.</p>',
        status: 'active',
    }
];

const inspectionQuestions2 = [
    {
        id: 47,
        inspection_id: 149,
        viq: '3.1.1.',
        tag: 'Core',
        chapter_no: 3,
        question: 'Were all personnel familiar with the safety management system regarding emergency response procedures, and was evidence available that drills and exercises had been conducted in accordance with the ISM Code?'
    },
    {
        id: 48,
        inspection_id: 149,
        ...
        tag: 'Core',
        chapter_no: 3,
        question: 'Were the Master and officers familiar with the company policy on drug and alcohol abuse and the procedures for testing and reporting?'
    },
    {
        id: 49,
        inspection_id: 149,
        ...
        tag: 'Rotational 1',
        chapter_no: 5,
        question: 'Was the vessel fitted with fire-fighting equipment appropriate to the vessel type, and was the equipment correctly maintained and ready for immediate use?'
    },
    {
        id: 50,
        inspection_id: 149,
        ...
        tag: 'Core',
        chapter_no: 5,
        question: 'Were the Master and officers familiar with the company procedure for safety inspections of the forecastle, and had inspections been effective in identifying hazards to health, safety and the environment?'
    },
]

const inspectionScores2 = [
    {
        question_id: 47,
        negative: 'Not as expected',
        noc: 'Lack of training/familiarisation',
        soc: 'Emergency Response Procedures',
        remark: 'A junior officer demonstrated poor knowledge of the procedure for an engine room fire, specifically the sequence for engaging the fixed fire-fighting system.',
        category: 'human',
        operator_comments: [{"date":"05 Sep 2025 21:41","name":"Captain ABC","immediateCause":"Familiarization Checklist SF 10 was completed but not signed by the safety officer for the onboard chief engineer. The remaining were observed duly completed. The Drug and Alcohol, Medical records were not signed by the same crew member. All other forms were observed and signed by the required personnel. The familiarizations had though been completed as required.","rootCause":"Lack of compliance with company procedures","correctiveAction":"The SF 10 for the officer was rectified immediately, the Safety officer completed the familarization checklist by validating it. The concerned officer was briefed by master,the importance of the personal documents being thoroughly verified and countersigned. The importance of adhering to company procedures with respect to SMS was reiterated to all crew members.","preventativeAction":"The master carried out training for all on board with respect to following company procedures. The effectiveness of compliance has to be checked by senior officers by virtue of routine audits during month end documentation. The officer will be called to office prior his next assignment where the technical superintendent will discuss the company procedures, and the work arrangements to be followed onboard.","category":"human"},{"date":"05 Nov 2024 21:41","name":"Captain Umesh Catoor","immediateCause":"Familiarization Checklist SF 10 was completed but not signed by the safety officer for the onboard electrical officer. The remaining were observed duly completed. The Drug and Alcohol, Medical records were not signed by the same crew member. All other forms were observed and signed by the required personnel. The familiarizations had though been completed as required.","rootCause":"Lack of compliance with company procedures","correctiveAction":"The SF 10 for the Electrical officer was rectified immediately, the Safety officer completed the familarization checklist by validating it. The electrical officer was briefed by master,the importance of the personal documents being thoroughly verified and countersigned. The importance of adhering to company procedures with respect to SMS was reiterated to all crew members.","preventativeAction":"The master carried out training for all on board with respect to following company procedures. The effectiveness of compliance has to be checked by senior officers by virtue of routine audits during month end documentation. The officer will be called to office prior his next assignment where the marine superintendent will discuss the company procedures, and the work arrangements to be followed onboard.","category":"human"}],
        score: 4,
        isNegative: 'yes'
    },
    {
        question_id: 48,
        negative: 'Observable or detectable deficiency',
        noc: 'Maintenance deferred – awaiting spares',
        soc: 'Fire Hoses and Nozzles',
        remark: 'One fire hose in the forepeak was found to have a small leak during a pressure test, and the replacement was on order, documented in the defect list.',
        category: 'hardware',
        tmsa: '1A.1.5',
        score: 5,
        isNegative: 'yes'
    ...
    {
        question_id: 49,
        negative: 'No observation to support expected condition',
        noc: 'Lack of policy/procedure',
        soc: 'D&A Policy documentation',
        remark: 'The vessel did not have the latest revision of the company Drug and Alcohol policy, which had been updated 3 months prior.',
        category: 'process',
        tmsa: '1A.1.5',
        score: 6,
        isNegative: 'yes'
    ...
    {
        question_id: 50,
        negative: 'Exceeded normal expectation',
        noc: '7. Workplace ergonomics incl. signage, tools, layout, space, noise, light, heat, etc',
        soc: 'Senior Deck Officer',
        remark: 'Emergency portable framo pump located in the forecastle deck was secured within a dedicated steel bucket preventing any unexpected damages.',
        category: 'human',
        score: null,
        isNegative: 'no',
        pif: [{"pifNumber":"7","pifDescription":"Workplace ergonomics incl. signage, tools, layout, space, noise, light, heat, etc."}],
        human_name: "Chief engineer ABCD",
        crew_id : 3,
    },
]
